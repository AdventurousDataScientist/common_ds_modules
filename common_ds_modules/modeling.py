...
def evaluate(train, y_train, model):
    rmse_scores = cross_val_score(model, train, y_train, scoring='neg_root_mean_squared_error', cv=5)
    print(f'mean RMSE = {-np.mean(rmse_scores)}, std RMSE = {np.std(rmse_scores)}')
    
    #mae_scores = cross_val_score(model, train, y_train, scoring='neg_mean_absolute_error', cv=5)
...
def get_lasso_regression_model(train_model_data_final, y_train, lasso_param_grid):
    lasso_gs = GridSearchCV(Lasso(), lasso_param_grid, cv=5, scoring='neg_root_mean_squared_error', verbose=2)
...
def get_ridge_regressor_model(train_model_data_final, y_train, ridge_param_grid):
    ridge_gs = GridSearchCV(Ridge(), ridge_param_grid, cv=5, scoring='neg_root_mean_squared_error', verbose=2)
...
def get_elastic_net_regressor_model(train_model_data_final, y_train, elastic_net_param_grid):
    elastic_net_gs = GridSearchCV(ElasticNet(), elastic_net_param_grid, cv=5, scoring='neg_root_mean_squared_error', verbose=2)
...
def get_knn_regressor_model(train_model_data_final, y_train, knn_param_grid):
    knn_grid_search = RandomizedSearchCV(KNeighborsRegressor(), knn_param_grid, cv=5, n_iter=MAX_MODELS, verbose=2)
